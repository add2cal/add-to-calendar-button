...
- Supporting 44+ languages, incl. RTL text for Arabic or Persian; but also custom labels and text blocks (i18n).
...
If you want to use a non-default style or non-English language, you would import them separately.
...
You can then configure the button by adding the options as attributes to it.  
Boolean values (true/false) can be set as `option-name="true"` or simply `option-name`. Not setting it at all would be automatically translate to "false".
...
**IMPORTANT NOTE:** Run `npm install` and `npm run fix` to auto-lint!
