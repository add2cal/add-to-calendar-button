...
- Use the `recurrence` option to define recurring events. But mind that this will deactivate the Yahoo, Microsoft365, Teams, and Outlook options, since they do not support it at the moment (users could still use iCal in this case). 
  ...
  - Or you define one of the following more simple rules: hourly, daily, weekly, monthly, yearly. If you go that way, you can enrich your rule with one of the following specifics:
    - `recurrence_freq` to specify the frequency. "1" would be the default, while "3" would mean "every third".
    - `recurrence_endDate` to specify an end date. This should be the exact date of the final event instead of a random future day.
    ...
    - `recurrence_byDay` to specify the weekdays (MO, TU, WE, TH, FR, SA, SU), where the even occurs (if, for example, it is bound to Tuesday instead of the 24th). Can be an array like "[TU, FR]".
    - `recurrence_byMonth` to specify the months (1, 2, 3, ... 12), where the event should happen.
    - `recurrence_byMonthDay` to specify a numbered day (1,2, ... 31) instead of a weekday.
