...
> **Status: COMPLETE. Do not transfer this file to the main repository.**
> This document is refactor HISTORY - phase notes, gates, and decisions of a finished
> project. Everything evergreen has moved to its living home: architecture and the
> contracts-vs-internals guidance to `.ai/ARCHITECTURE.md`, release mechanics to
> `.ai/RELEASE.md`, test strategy and case lists to `.ai/TEST-STRATEGY.md` /
> `.ai/TEST-CASES.md`, consumer-facing material to `.ai/MIGRATION-V3.md` and
> `.ai/RELEASE-NOTES-V3.md`, and repo working knowledge to `CLAUDE.md`.
> On the merge to main: bring the living documents, leave this file behind (it stays
> retrievable in the fork's history). Nothing in the code references it anymore.
...
- [x] Phase 10 - Hardening
  - Security review results: (1) lit templates - all unsafeHTML sites audited: static icon markup, translate-hook output (page-author-controlled by contract: labels/customLabels support pseudo-element formatting since v2) and the recurring glyph; third-party-ish data (name, location, descriptionHtmlFree) renders through TEXT interpolation only. (2) NEW url scheme allowlist in atcb_secure_url (http/https/webcal/webcals/mailto/intent/data:text-calendar + relative; control-char-tolerant sniffing like browsers) on top of the existing traversal check. (3) description [url] linkification hardened: scheme allowlist (http/https/webcal/webcals/mailto), full attribute + text escaping - quote breakouts impossible; unsafe schemes render as plain text. (4) prototype pollution: the PRO override merge was already safe by construction (closed allowlist iteration, shallow assignment, hasOwnProperty-guarded); defense in depth added anyway - atcb_strip_unsafe_keys removes **proto**/constructor/prototype recursively at EVERY json boundary (atcb_secure_content json path, both attribute JSON.parse sites, inline slot json, PRO response). (5) schema.org rich data now serializes all interpolated values via JSON.stringify (quotes/backslashes in fields no longer break the JSON-LD; script sits in textContent, no execution surface). (6) npm audit: 0 vulnerabilities. (7) static scans: semgrep p/javascript + p/typescript + p/security-audit = 0 findings (one false positive in the test harness annotated with justification); eslint-plugin-security active in the lint gate. New test group SEC (5 cases) pins allowlists, escaping, pollution-immunity end-to-end through the attribute parser, and rich-data JSON validity.
  - Memory-leak review results: teardown architecture verified sound - bodyScheme MutationObservers disconnect + deregister on disconnect (and pre-disconnect on re-registration), store entries delete on disconnect, schema scripts and shadow styles/links/wrappers removed, modal host + body scroll lock removed even when unmounting with an open modal; global keyup/keydown/resize listeners are deliberate page-level singletons registered exactly once (stateless: they resolve the active button dynamically). atcb_action custom-trigger hosts are replaced on re-trigger (no owner lifecycle - v2 contract). New test group MEM (4 cases): singleton registration count, observer disconnect accounting via instrumented MutationObserver, open-modal unmount teardown, and 30 mount-unmount cycles asserting zero DOM debris plus heap growth < 2 MB under forced GC (test runner now launches Chrome with --js-flags=--expose-gc).
  - Size report (v2.15.0 PUBLISHED artifacts vs v3 --min build): atcb.js 542.1/94.9 -> 366.6/85.1 KB (raw/gzip, -32%/-10%); module 501.1/85.2 -> 280.1/69.1 (-44%/-19%, lit external); cjs 493.1/90.0 -> 331.1/82.3 (-33%/-9%, lit bundled); NEW atcb.min.js 229.8/69.5 (v2.15 tarball shipped no min file - jsdelivr switch to min = maintainer decision, noted in the migration guide); NEW ssr 143.3/22.6 server-only; 7 style deltas 82.1 KB raw total, 25 locale packs 71.5 KB raw total, all lazy; npm tarball 1.1 MB -> 630 KB packed, 5.4 MB -> 2.9 MB unpacked; vite consumer app with one style + one locale: 233 KB minified. Size goals met (module gzip target and the per-style delta waivers from phase 5 carry over unchanged).
  - Migration guide drafted at .ai/MIGRATION-V3.md (consumer-facing changes, deliberate breaking changes, size report, documentation-pages handoff list for the website team).
  - Gate: full suite green on both browsers (see below), test:package green, test:ssr green.
- [x] Phase 11 - Documentation and release preparation
  - Release notes drafted at .ai/RELEASE-NOTES-V3.md (consumer-facing, written to ship as-is for the GitHub release and announcement; includes a pre-release intro note and the npm next install command)
  - CHANGELOG.md gained the "Version 3" section in the file's established format (one v3.0 entry with nested bullets, breaking changes marked with the warning emoji, in the voice of the v2.0 block)
  - demo CONTENT updated (strictly no technical changes; demo lint + prettier + nuxi typecheck + full `nuxt generate` all green): use page CDN snippet moved to @3; configuration page gained the kebab-case intro note (en+de), on-demand loading notes at buttonStyle and language (incl. the corrected language list with he and uk, full-locale support), the customLabels key renames plus a fixed link to the new locale files, and NEW styleSource/loadAllStyles reference rows (also added to the search index); advanced-use case 11 (loading strategies) REWRITTEN for the v3 loading model (the described no-pro/unstyle variants are retired), NEW case 12 for server-side rendering (en+de, with sidebar link and i18n keys); the nextjs/astro/nuxt framework guides got a shared ssr_note pointing at case 12
  - scripts/set-version.mjs now accepts prerelease versions (x.y.z-tag.n explicit, `prerelease` counter bump, patch-finalizes-prerelease npm semantics) and got two latent bugs fixed along the way: the version-constant replacement pattern had been dead since the TS migration (type annotation broke it) and the src walk had been dead since the phase 3 restructure (never descended into subdirectories) - the full lifecycle 2.15.0 -> 3.0.0-next.1 -> next.2 -> 3.0.0 is exercised and green
  - assets/css regenerated files ride along in this commit (they carry the phase 9 clipboard-input rule; generated artifacts, previously unpushed)
  - Gate: extended 274/274, lint clean, demo generate green
- [x] Phase 12 - Cleanup and AI docs
  - Comment sweep: all refactor/phase/plan references removed from code comments across src/, scripts/ and tests (24 rewrites plus the 10 generated css source banners) - every remaining comment is evergreen and explains the code as it is; kept on purpose: living-doc pointers (.ai/TEST-CASES.md in test headers), compatibility-contract references to v2 (the compat oracle, legacy alias semantics), and the version name where it describes current behavior; zero references to this plan remain in code
  - Evergreen docs: NEW .ai/ARCHITECTURE.md (module map, init/interaction data flow, state, style/i18n pipelines, SSR + hydration, build outputs, contracts-vs-internals guidance) and NEW .ai/RELEASE.md (artifacts, version tooling incl. prerelease flow, gates, next-tag flow, demo relationship)
  - Recommendation recorded in this file's header: the plan is history and must NOT travel to main; the living documents carry everything evergreen
  - CLAUDE.md rewritten for the current architecture: Active-project section removed, superseded v2 notes dropped (Grunt build, no-TypeScript rule), stack/build/testing sections updated, file map extended (ssr, compat, sizes), new gotchas from the build and test infrastructure (build string-replacement hooks, script-base derivation, raw-json middleware, lit node condition, DSD adoption, expected test counts per tier), pitfalls re-pointed at current file names
  - Gate: comments-only proof - tsc clean, build green, full suite 484/484 on chrome-headless-shell and full Chrome (one non-deterministic flake was observed on full Chrome across the phase 10-12 runs, each time passing on rerun and never on headless-shell; worth watching during review iterations)
