...
- Setting the option `buttonsList` to "true", will skip the list completely and render individual buttons per option next to each other.
...
- Clicking an option marks the event as "saved" by adding a checkmark icon (not, if the button's text labels are turned off or when using atcb_action). You can deactivate this functionality by setting the option `"checkmark":"false"`.
