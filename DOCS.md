...
- There is almost no static text block. For the tiny little rest, you can define a translation by using the `language` option and providing an [ISO 639-1 code](https://www.w3schools.com/tags/ref_language_codes.asp). Supported options and therefore language codes: English (en, default), German (de), Dutch (nl), French (fr), Spanish (es), Portugese (pt), Turkish (tr), Chinese (zh), Hindi (hi), Polish (pl), Indonesian (id), Norwegian (no), Finnish (fi), Swedish (sv), Czech (cs), Japanese (ja), Italian (it), Korean (ko), Vietnamnese (vi), Arabic (ar, incl RTL). If you provide a full language+country code, the script automatically only takes the first two characters.
...
- ics files are generated on the fly. However, if you want to go more stable, you can also explicitly define a self-hosted file, setting its absolute path (starting with https://) with the `icsFile` option. Mind that the custom file name would not work in that case.
- You can use the option `inline` in order to make the button appear with inline-block instead of block style.
...
- Use `hideBackground` if you want to be the background overlay to be fully transparent.
- If you want to hide the icons and go more text-only, use the options `hideIconButton`, `hideIconList`, and `hideIconModal` to hide them at the specific locations.
- The text label at the button as well as at the list items can be omitted via the options `hideTextLabelButton` and `hideTextLabelList`.
- You can change the default button style with the `buttonStyle` option. Valid themes would be "3d", "round", "flat", "text", "neumorphism", "date", "custom", and "none". "none" would simply load no css at all. "custom" would look for any external css file, defined via the `customCss` option.
- Setting the option `buttonsList`, will skip the list completely and render individual buttons per option next to each other (except for the buttonStyle "date").
...
- If you set at least a name, startDate, and location, the script automatically generates schema.org rich data (except if you are using the atcb_action function). Use a URL for the location and it will be labeled as online event. Set the option `hideRichData` to disable this feature.
...
- Clicking an option marks the event as "saved" by adding a checkmark icon (not, if the button's text labels are turned off, you use the buttonsList option, or when using atcb_action). You can deactivate this functionality by setting the option `hideCheckmark`.
...
- As an alternative to providing a specific predefined event, you can also host a calendar and offer it for subscription (requires a hosted service). Offering the subscription via the button, you need to provide an explicit `icsFile` and set `subscribe`. "Name" and "startDate" are still required for organizational purposes, but every other event parameter can be skipped in the subscription case. For Microsoft services, the "Name" will be used as name for the calendar! Mind that this will disable the MS Teams option and it is a not completely stable feature yet!
- For users loading the button on their iPhone within a WebView environment (e.g. the Instagram Browser), we are not able to directly offer the ics file for download. Therefore, we show a small instruction note on how to do it instead. If you are using the button in your own app and WebView, where you actively allow the download of ics files and are also providing the file explicitely with the `icsFile` option, you can bypass this workaround with the option `bypassWebViewCheck`.
- Per default, there is a slight branding. This supports the open-source project. If you still want to remove it, simply specify the option `hideBranding`.
