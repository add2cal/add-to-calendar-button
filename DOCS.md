...
- Setting the option `buttonsList` to "true", will skip the list completely and render individual buttons per option next to each other (except for the buttonStyle "date").
...
- If you set at least a name, startDate, and location, the script automatically generates schema.org rich data (except if you are using the atcb_action function). Use a URL for the location and it will be labeled as online event. Set the option `"richData":"false"` to disable this feature.
...
- Clicking an option marks the event as "saved" by adding a checkmark icon (not, if the button's text labels are turned off, you use the buttonsList option, or when using atcb_action). You can deactivate this functionality by setting the option `"checkmark":"false"`.
