...
- Define multi-date events by using the `dates` option, which holds an array of objects, which needs to hold the UID, date, and time information of the sub-event. It can optionally also hold their own name, description, location, timeZone, organizer, sequence, and status. Mind that in this case, the name at the top level becomes the name of the event series and is still required.
