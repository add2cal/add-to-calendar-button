...
| **optionsIOS vs optionsMobile (FIXED in this branch)** | Was: on iOS, `optionsMobile` overrode `optionsIOS` when both were set — contradicting the docs. Fixed in `src/atcb-decorate.js#atcb_determine_options_source`: `optionsIOS` now takes precedence; `optionsMobile` is the fallback. L-11 asserts the corrected behavior. |
| **Case-sensitive swap guard (FIXED in this branch)** | Was: the iCal↔Apple swap suppression checked `options.includes('ical')` on RAW values, so `optionsIOS="['iCal']"` (docs casing) got swapped while lowercase didn't. Fixed: the check now runs on `atcb_normalize_option_name`-normalized values. L-18/L-18b assert case-insensitive behavior. |
...
| Test-infra notes | (1) Several Chrome builds crash the renderer when the shadow root of a FAILED-init element is accessed (headless-shell v150: `querySelector`; other builds: other accessors) - failed-init tests never touch the shadow root and assert via the `initFailed()` attribute contract + console capture instead. (2) `window.fetch` mocks MUST pass unknown URLs through to the original fetch - the test runner itself communicates via fetch. (3) PRO server payloads are pre-structured with a `dates` array - fixtures must mirror that. (4) **Runner concurrency is 1 by design**: Chrome's headless mode intensively throttles timers in BACKGROUNDED tabs, stalling the component's deferred init (`whenInitialized()` timeouts / "browser disconnected") for whichever test file's tab is not foreground - reproducible 100% with any two concurrent files on regular Chrome; chrome-headless-shell (no tab visibility) is unaffected. The usual disable-throttling switches did not reliably lift it. Serialized pages are deterministic everywhere; override for experiments via `WTR_CONCURRENCY`. |
