    ...
    <Listbox :modelValue="multiselect ? (Array.isArray(modelValue) ? modelValue : []) : modelValue" :multiple="multiselect" class="ml-2 py-2" @update:model-value="$emit('update:modelValue', $event)">
      <div class="relative w-auto">
        <ListboxButton
          ...
          class="focus-visible:ring-secondary/75 relative grid w-full cursor-pointer rounded-md bg-zinc-50 py-2 pl-3 text-left text-sm shadow hover:bg-white hover:shadow-md focus:outline-none focus-visible:ring dark:bg-zinc-700 dark:hover:bg-zinc-600"
        >
          ...
          <span v-if="!hasEmptyValue && clearable" class="z-1 absolute inset-y-0 right-0 flex items-center pr-7">
            <XMarkIcon class="hover:text-secondary h-5 w-5 text-gray-400" aria-hidden="true" @click.prevent="clear" />
          </span>
        ...
        <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
          <ListboxOptions class="ring-secondary/75 absolute z-10 mt-2 max-h-36 w-full overflow-y-auto overflow-x-hidden rounded-md bg-white py-1 text-sm shadow-lg ring focus:outline-none dark:bg-zinc-700">
            <ListboxOption
