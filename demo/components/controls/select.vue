    ...
    <Listbox :modelValue="multiselect ? (Array.isArray(modelValue) ? modelValue : []) : modelValue" :multiple="multiselect" class="ml-2 py-2" @update:model-value="$emit('update:modelValue', $event)">
      <div class="relative w-auto">
        <ListboxButton
          :aria-label="label"
          :class="[
            'focus-visible:ring-secondary focus-visible:ring/75 grid w-full cursor-pointer rounded-md bg-zinc-50 py-2 pl-3 text-left text-sm shadow hover:bg-white hover:shadow-md focus:outline-none dark:bg-zinc-700 dark:hover:bg-zinc-600',
