  ...
  <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_192px]">
    <div class="pr-0 lg:pr-8 xl:pr-12 2xl:pr-20">
      ...
      <div class="my-8 overflow-x-auto rounded-lg shadow-sm">
        <table>
          <thead>
            <tr>
              <th scope="col" class="py-3 px-3 font-semibold sm:px-5">
                {{ t('content.config.name') }}<span class="block pt-1 normal-case md:hidden">({{ t('content.config.value') }})</span>
              </th>
              <th scope="col" class="hidden py-3 px-3 font-semibold sm:px-5 md:table-cell">{{ t('content.config.value') }}</th>
              <th scope="col" class="py-3 px-3 font-semibold sm:px-5">{{ t('content.config.details') }}</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div class="mt-14 border-t border-zinc-300 pt-14 dark:border-zinc-700">
        <div class="mx-auto grid grid-cols-1 gap-12 md:grid-cols-3">
          <div class="next-group">
            ...
            <RouterLink class="button-primary next-button" v-else :to="{ name: 'home-i18n', hash: '#demo', params: { locale } }">
              {{ t('labels.startPlaying') }}
              <ArrowRightIcon class="next-icon" aria-hidden="true" />
            </RouterLink>
          </div>
    ...
    <div class="hidden border-l border-zinc-300 pl-8 text-sm dark:border-zinc-700 lg:block">
      <div class="sticky top-0 pt-4">
        ...
        <a href="#event-parameters" class="side-nav">{{ t('content.config.event_params') }}</a>
        <a href="#style-parameters" class="side-nav">{{ t('content.config.style_params') }}</a>
        <a href="#additional-parameters" class="side-nav">{{ t('content.config.additional_params') }}</a>
      </div>
    </div>
  </div>
</template>
