  ...
  <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_192px]">
    <div class="pr-0 lg:pr-8 xl:pr-12 2xl:pr-20">
      ...
      <div v-else>
        ...
        <p>
          Speziellere Beispiele und Erläuterung von komplexeren Konfigurationen findest du in der <RouterLink :to="{ name: 'advanced-use', params: { locale } }">"{{ t('navigation.advanced-use') }}"</RouterLink>.
        </p>
      </div>
      <section id="1">
...
&gt;&lt;/add-to-calendar-button&gt;
        ...
        <p v-if="locale=='en'">
          ...
          Check the <RouterLink :to="{ name: 'configuration', hash: '#recurrence', params: { locale } }">"{{ t('navigation.configuration') }}"</RouterLink> page for more details on that.
        </p>
        <p v-else>
          Alternativ zu der RRULE kannst du aber auch die "sprechenderen" Optionen nutzen.<br />
          Die <RouterLink :to="{ name: 'configuration', hash: '#recurrence', params: { locale } }">"{{ t('navigation.configuration') }}"</RouterLink>-Seite hält mehr Details dazu bereit.
        </p>
        <div class="block w-full justify-between md:flex">
          <div class="flex w-full flex-none justify-center p-6 pt-8 md:w-[300px]">
            <add-to-calendar-button
...
&gt;&lt;/add-to-calendar-button&gt;
      ...
      <section id="6">
        ...
        <div v-else>
          ...
          <p class="text-sm italic">
            Weitere System-Texte können mit der "customLabels"-Option verändert werden. Die <RouterLink :to="{ name: 'configuration', hash: '#customlabels', params: { locale } }">"{{ t('navigation.configuration') }}"</RouterLink>-Seite weiß mehr.
          </p>
        </div>
        <div class="block w-full justify-between md:flex">
          <div class="flex w-full flex-none justify-center p-6 pt-8 md:w-[300px]">
...
&gt;&lt;/add-to-calendar-button&gt;
    ...
    <div class="hidden border-l border-zinc-300 pl-8 text-sm dark:border-zinc-700 lg:block">
      <div class="sticky top-0 pt-4">
        <a href="#1" class="my-4 block">#1: {{ t('content.examples.1_short') }}</a>
        <a href="#2" class="my-4 block">#2: {{ t('content.examples.2_short') }}</a>
        <a href="#3" class="my-4 block">#3: {{ t('content.examples.3_short') }}</a>
        <a href="#4" class="my-4 block">#4: {{ t('content.examples.4_short') }}</a>
        <a href="#5" class="my-4 block">#5: {{ t('content.examples.5_short') }}</a>
        <a href="#6" class="my-4 block">#6: {{ t('content.examples.6_short') }}</a>
        <a href="#7" class="my-4 block">#7: {{ t('content.examples.7_short') }}</a>
        <a href="#8" class="my-4 block">#8: {{ t('content.examples.8_short') }}</a>
        <a href="#9" class="my-4 block">#9: {{ t('content.examples.9_short') }}</a>
      </div>
    </div>
  </div>
</template>
