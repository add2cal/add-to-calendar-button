import { rewrite_html_elements } from '../core/text';
...
const pendingLocaleLoads: Map<string, Promise<boolean>> = new Map();
...
function flatten_translations(strings: NestedTranslations, prefix: string = ''): { [key: string]: string } {
 ...
 */
function register_locale(language: string, strings: NestedTranslations | { [key: string]: string }): void {
  i18nStrings[`${language}`] = { ...calendarNames, ...flatten_translations(strings as NestedTranslations) };
}

function locale_base_url(data: ATCBConfig): string {
 ...
 */
async function ensure_locale(data: ATCBConfig): Promise<void> {
...
function translate(identifier: string, language?: string): string {
...
export { translate_hook, translate, register_locale, ensure_locale, availableLanguages, rtlLanguages, i18nStrings };
