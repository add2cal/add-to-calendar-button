...
function atcb_log_event(event: string, trigger: string, identifier: string): void {
  const parentEl: Element | null = (function () {
...
function atcb_push_to_data_layer(event: string, trigger: string): void {
  let action: string = '';
  switch (event) {
    case 'initialization':
      action = 'Initialized';
