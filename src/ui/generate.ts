...
import { load_css, set_light_mode } from '../element/index';
...
function close_modal_list_if_idle(host: ShadowRoot, data: ATCBConfig): void {
...
function generate_label(host: ShadowRoot, data: ATCBConfig, parent: HTMLElement, type: string, icon: boolean = false, text: string = '', oneOption: boolean = false): void {
    ...
    default:
      ...
      if (!data.blockInteraction) {
        ...
        parent.addEventListener(
          'touchend',
          debounce_leading((event: TouchEvent) => {
            event.preventDefault();
            if (type === 'rsvp' && typeof generate_rsvp_form === 'function') {
        ...
        if (data.trigger === 'click' || (type === 'rsvp' && typeof generate_rsvp_form === 'function')) {
          parent.addEventListener(
            'mouseup',
            debounce_leading((event: MouseEvent) => {
              event.preventDefault();
              if (type === 'rsvp' && typeof generate_rsvp_form === 'function') {
                generate_rsvp_form(host, data, parent);
              } else {
        ...
        parent.addEventListener(
          ...
          debounce_leading(async () => {
            if (oneOption) {
              ...
              log_event('openCalendarLink', parent.id, data.identifier as string);
            }
...
function generate_label_content(data: ATCBConfig, parent: HTMLElement, type: string, icon: boolean, text: string, oneOption: boolean): void {
  ...
  if (text === '') {
    if (data.options!.length === 1 || type === 'trigger') {
      ...
      text = translate_hook('close', data);
    } else {
      text = translate_hook(type, data);
    }
  }
...
function generate_dropdown_list(host: ShadowRoot, data: ATCBConfig): HTMLDivElement {
...
function generate_bg_overlay(host: ShadowRoot, trigger: string = '', modal: boolean = false, darken: boolean = true, closable: boolean = true): HTMLElement {
 ...
 */
function create_atcbl(host: ShadowRoot, atList: boolean = true, returnEl: boolean = false, licenseNoteOnly: boolean = false): HTMLDivElement | null | void {
    ...
    switch (button.type) {
      ...
      case '2timeslink': // for the note that the user shall click the button twice
        modalButton.addEventListener(
          ...
          debounce(async () => {
            close(mainHost);
            await generate_links(mainHost, goto.type as string, data, goto.id as string, keyboardTrigger, false, true);
          }),
        );
        (modalButton as HTMLElement).addEventListener('keyup', async function (event: KeyboardEvent) {
          if (event.key === 'Enter' || event.code == 'Space') {
...
async function generate_modal_host(host: ShadowRoot, data: ATCBConfig, reset: boolean = true): Promise<ShadowRoot> {
...
async function generate_overlay_dom(host: ShadowRoot, data: ATCBConfig): Promise<Element | null> {
